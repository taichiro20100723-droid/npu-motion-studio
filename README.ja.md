...
曲の入った動画を8秒ごとのカットに分け、音量の山に合わせて動きを変えながら、NPU画像とArc GPU補間で
長いMVにします。学校→教室→図書館→ステージのように、前の画面から次の画面へ大胆に変わる指示と、
ワイプ・ズーム・円形開閉などの自動トランジションが入ります。歌詞のタイミングがある場合は `.lrc` ファイルを
指定できます。歌詞がない場合も、音量の山を使って自動でカットを作れます。完成版は人物なしの素材を
中心にし、実在人物の写真は入力しません。人物を使う派生版でも、実在人物に似せないAI生成だけにします。

```powershell
.\.venv\Scripts\python.exe scripts\make_music_video.py "曲.mp4" `
  --backgrounds "顔のない背景写真フォルダー" --photo-every 4
```

今回の早稲田佐賀祭サンプルは、`make_waseda_saga_festival_mv.bat` をダブルクリックすると、同じダイナミック設定で作れます。
学校写真は校舎・校庭・海側など顔のない素材だけを使い、AIカットにも実在生徒の顔を入力しません。
公式サイトの写真を公開動画へ使うときは、学校の許可・写真の利用条件を確認してください。
