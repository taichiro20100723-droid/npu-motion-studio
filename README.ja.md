...
- **変形文字モード**では、普通の文字をALIEN／RUNE／SIGNAL／CYBER風のグリフへ変換し、そのまま動画化できます。
- 変形文字はコピー用TXT、編集できるアニメーションSVG、インストールできるTTFフォントとして保存できます。
...
上部の「変形文字を動かす」を選び、元の文字（例: `NPU MOTION`）と雰囲気を選びます。
「素材を作る」で同じグリフのプレビューを確認し、TXT・SVG・フォントを個別に保存できます。
そのまま「動画を作る」を押すと、作ったグリフを入力画像としてNPU動画にします。元の文字と
スタイルが同じなら、あとから別の動画を作っても同じ文字形を再現できます。
