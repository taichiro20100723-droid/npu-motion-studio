...
> **5秒で見るデモ：** 巨大ロボットが、本物の柴犬へ変身します。最初の画像と最後の画像は自分で決め、その「ありえない途中」だけをIntelノートPCの中で生成します。

[MP4を見る](examples/robot-to-dog/robot-to-dog.mp4) · [Motion Brush版を見る](examples/motion-brush/robot-to-dog-motion-brush.mp4) · [自分の作品を共有する](SHOWCASE.md)

この仕組みをもっと見たいと思ったら、**⭐ Starを付けて、いちばん変なA→B作品を投稿してください。**
...
> フル動画拡散モデルではありません。文章に合わせたNPU途中画像、動きのワープ、GPU補間、ハードウェアエンコードを組み合わせた、意図的に軽いハイブリッド方式です。

## 10秒でわかる仕組み

| 入力 | 出力 |
|---|---|
| Aの画像＋Bの画像 | AとBを正確な端点にした変身動画 |
| 途中の動きを一文 | 単純なクロスフェードではない、動き付きのNPU画像 |
| Intel Core UltraノートPC | NPU＋Arc GPU＋Quick Syncで作るローカルMP4 |

「最初と最後がわかる」「途中が意外」「短いMP4で見せやすい」ことを重視しています。MV、ショート動画、作品の発表、製品コンセプト、ミーム、映像実験に向いています。
...
始点と終点を守り、途中の要所をNPUで描き、動きのワープと補間を組み合わせる方式です。大きく違うものへの変身では途中がシュールになることがあります。それも隠さず楽しむための設計です。
...
変な作品ができたら、[SHOWCASE.md](SHOWCASE.md)に沿ってA画像、B画像、使った文章、機器、生成時間、驚いた点を投稿してください。完璧な成功だけでなく、変で美しい失敗も歓迎します。
